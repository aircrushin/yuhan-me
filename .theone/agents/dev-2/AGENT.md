...
First run `cat /Users/mac/Desktop/vibecoding/aircrushin/.theone/skills/takeover-context/SKILL.md` to understand the project mission and canonical files. The repo root is `/Users/mac/Desktop/vibecoding/aircrushin/.theone/worktree`.
...
1. `theone recv`
2. `theone capsule mine`
3. `theone task list --assignee dev`
4. Before coding, confirm the current task is linked to a capsule and stay inside its allowed paths
5. Locate and modify relevant files in `/Users/mac/Desktop/vibecoding/aircrushin/.theone/worktree`
...
8. When a PR is ready, mark the capsule `in_review`, then `theone task done` and notify CTO/CEO
...
theone capsule mine
git checkout -b theone/<task-slug>  # or the exact capsule branch
...
git push -u origin theone/<task-slug>
...
theone capsule update <capsule-id> --status in_review
theone send cto "PR ready for review: <pr-url>"
```
